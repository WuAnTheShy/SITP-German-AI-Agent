...
const Login = () => {
  ...
  const [showCaptcha, setShowCaptcha] = useState(
    () => localStorage.getItem("captcha_unified") === "true",
  );
  const [captcha, setCaptcha] = useState({ code: "", dataUrl: "" });

  // 🚦 登录限制状态
  const [failedAttempts, setFailedAttempts] = useState(() =>
    parseInt(localStorage.getItem("failed_attempts_unified") || "0", 10),
  );
  const [lockoutUntil, setLockoutUntil] = useState(() =>
    parseInt(localStorage.getItem("lockout_until_unified") || "0", 10),
  );
  ...
  const isDisabledError = /登录被停用|账号已被管理员停用|账号已被停用/.test(
    error || "",
  );
  ...
  //       setError(
  //         `由于多次尝试失败，账号已锁定。请在 ${formatTime(lockoutRemaining)} 后重试`,
  //       );
  //       return;
  //     }
  ...
  //       if (studentResult.ok) {
  //         // 后端统一返回 user.role,前端按真实角色处理(不再写死 'student')
  //         const realRole =
  //           studentResult.data?.user?.role ||
  //           studentResult.data?.data?.user?.role ||
  //           "student";
  //         handleLoginSuccess(realRole, studentResult.data);
  //         return;
  //       }

  //       const studentBlocked =
  //         studentResult.code === 403 &&
  //         /停用|等待审核|已被拒绝/.test(studentResult.message || "");
  //       if (studentBlocked) {
  ...
  //         teacherResult.code === 403 &&
  //         /停用|等待审核|已被拒绝/.test(teacherResult.message || "");
  //       if (teacherBlocked) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const encryptedPassword = await sha256Hex(formData.password);
      // 统一登录入口:后端会根据 user_no/username 自动识别角色
      const result = await attemptLogin(API_LOGIN, {
        username: formData.username,
        password: encryptedPassword,
      });
      ...
      if (result.ok) {
        const realRole =
          result.data?.user?.role || result.data?.data?.user?.role || "student";
        handleLoginSuccess(realRole, result.data);
        return;
      }

      // 业务上的 403(账号被停用/待审核/被拒绝)单独处理
      const blocked =
        result.code === 403 &&
        /停用|等待审核|已被拒绝/.test(result.message || "");
      if (blocked) {
        handleBusinessBlocked(result.message);
        return;
      }
  ...
  const handleLoginSuccess = (role, data) => {
    ...
    if (role === "student") {
      ...
      const isAdminToken =
        typeof token === "string" && token.startsWith("admin-token-");
      const isAdminAccount =
        (formData.username || "").trim().toLowerCase() === "admin";
      const actualRole =
        isAdminToken || (isAdminAccount && user)
          ? "admin"
          : user?.role || "teacher";
      ...
      const displayUser = {
        ...
        name:
          user?.name ??
          user?.display_name ??
          (actualRole === "admin" ? "管理员" : formData.username),
        role: actualRole,
      };
  ...
  return (
    <div className="min-h-screen theme-bg-premium flex flex-col items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="theme-glass-card rounded-2xl w-full max-w-md p-8 relative z-10 animate-fade-in-up delay-200 overflow-hidden">
        ...
        {error && (
          <div
            className={`mb-5 p-4 rounded-xl flex items-start ${isDisabledError ? "bg-amber-500/10 border border-amber-500/30" : "bg-red-500/10 border border-red-500/30"}`}
          >
            <AlertCircle
              className={`h-5 w-5 mr-2 flex-shrink-0 mt-0.5 ${isDisabledError ? "text-amber-500 dark:text-amber-400" : "text-red-500 dark:text-red-400"}`}
            />
            <div>
              <p
                className={`text-sm font-medium ${isDisabledError ? "text-amber-700 dark:text-amber-300" : "text-red-600 dark:text-red-300"}`}
              >
                {error}
              </p>
              {isDisabledError && (
                <p className="text-xs text-amber-700/80 dark:text-amber-200/90 mt-1">
                  账号当前被管理员停用，请联系管理员恢复“可登录”后再尝试。
                </p>
              )}
            </div>
          </div>
          ...
          >
            {isLocked ? (
              <span className="flex items-center">
                <Lock className="mr-2 h-5 w-5" /> 锁定中 (
              ...
              <span className="flex items-center">
                <ShieldCheck className="mr-2 h-5 w-5" /> 立即登录 / Anmelden
              </span>
            )}
          </button>
        </form>
        ...
        <div className="mt-8 pt-6 border-t border-gray-200 dark:border-white/10 text-center space-y-4">
          <div className="flex justify-center gap-6">
            <button
              ...
              className="text-sm text-blue-600 dark:text-blue-400 hover:text-blue-500 font-semibold flex items-center gap-1 transition-colors"
            >
              <UserPlus size={14} /> 学生注册
            </button>
            <button
              onClick={() => navigate("/teacher/register")}
              className="text-sm text-indigo-600 dark:text-indigo-400 hover:text-indigo-500 font-semibold flex items-center gap-1 transition-colors"
            >
