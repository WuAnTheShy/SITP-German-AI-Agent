    ...
    <StudentLayout>
      <div className="student-page">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex gap-2 bg-slate-100 dark:bg-slate-800/80 p-1 rounded-lg border border-slate-200 dark:border-slate-700/70">
            ...
            {mode === 'flashcard' && vocabList.length > 0 && (
              <div className="flex flex-col items-center">
                <div onClick={() => setIsCardFlipped(!isCardFlipped)}
                  className="student-card w-96 h-56 flex flex-col items-center justify-center cursor-pointer hover:shadow-xl transition-shadow p-8 select-none">
                  {!isCardFlipped ? (
                    <>
                ...
                <div className="flex gap-3 mt-6">
                  <button onClick={handlePrevCard} disabled={currentCardIndex <= 0}
                    className="student-action-secondary px-5 py-2 rounded-lg disabled:opacity-40 disabled:cursor-not-allowed transition-colors">上一个</button>
                  ...
                  <button onClick={handleNextCard} disabled={currentCardIndex >= vocabList.length - 1}
                    className="student-action-secondary px-5 py-2 rounded-lg disabled:opacity-40 disabled:cursor-not-allowed transition-colors">下一个</button>
            ...
            {mode === 'list' && (
              <div className="space-y-3">
                {vocabList.length === 0 ? (
                  ...
                  <div key={item.id} className="student-card flex items-center gap-4 p-4 hover:shadow-sm transition-shadow">
        ...
        <div className="mt-8 text-center">
          <button onClick={handleAIGenerateVocab} disabled={generating || loadingVocab}
            className="student-action-primary px-8 py-3 rounded-lg disabled:bg-gray-400 disabled:cursor-not-allowed font-medium text-lg">
