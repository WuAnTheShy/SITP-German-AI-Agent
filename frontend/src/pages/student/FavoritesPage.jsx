    ...
    <StudentLayout>
      <div className="flex-1 overflow-y-auto p-8">
        ...
        <div className="flex gap-3 flex-wrap mb-6">
          {cateLoading ? (
            <p className="text-gray-400 dark:text-gray-500">分类加载中...</p>
          ) : favCates.map(cate => (
            <button key={cate.id} onClick={() => setSelectedType(cate.type)} disabled={listLoading}
              className={`px-4 py-3 rounded-lg border-2 transition-all ${selectedType === cate.type ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400' : 'border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 hover:border-blue-300 text-gray-700 dark:text-gray-300'
                }`}>
              {cate.name}
            </button>
          ))}
        ...
        {listLoading ? (
          ...
          <div className="text-center py-16 text-gray-400 dark:text-gray-500">该分类暂无收藏内容，快去学习收藏吧！🌟</div>
        ) : (
